...
const DiseaseType = () => {
    ...
    const [editDiseaseTypeId, setEditDiseaseTypeId] = useState('');
    const [editDiseaseTypeDescription, setEditDiseaseTypeDescription] = useState('');
    //CREATE
    const createDiseaseType =  () => {
        axios({
            method: 'post',
            url: baseURL,
            data: { 
              id: diseaseTypeId,
              description: diseaseTypeDescription
            }
        }).then((response)=>{
            getAllDiseaseType();
            console.log(response)
            // alert("Disease Type Created")
            alert(response.data)
            setDiseaseTypeId('')
            setDiseaseTypeDescription('')
          }).catch((error)=>{
            console.log(error)
            });
    }
    ...
    const handleEdit = () => {
        axios({
            ...
            headers: {}, 
            data: {
              id: editDiseaseTypeId, 
              description: editDiseaseTypeDescription
            }
        }).then((response)=>{
            console.log(response)
            alert(response.data)
            // alert("Disease Type Updated")
            getAllDiseaseType();
          }).catch((error)=>{
            console.log(error)
            alert("Not Updated")
                                ...
                                <tr className="border-b border-gray-300 hover:bg-gray-200">
                                    ...
                                    <td className="py-3 px-3 text-center">
                                        <div className="flex item-center justify-center">
                                                                    ...
                                                                    <div className="w-full max-w-[570px] rounded-[20px] bg-white py-12 px-8 text-center md:py-[60px] md:px-[70px]">
                                                                        ...
                                                                        <div className="relative">
                                                                            ...
                                                                            <input 
                                                                                ...
                                                                                name="diseasetypeid"
                                                                                placeholder="DiseaseType ID" 
                                                                                onChange={(e)=>setEditDiseaseTypeId(e.target.value)}/>
                                                                        </div>
                                                                        ...
                                                                        <div className=" relative ">
                                                                            <label for="required-email" className="text-gray-700 left-0 top-0 flex">Description<span className="text-red-500 required-dot">*</span>
                                                                                ...
                                                                                onChange={(e)=>setEditDiseaseTypeDescription(e.target.value)}/>
                                                                        </div>
                                            ...
                                            <div className="w-4 mr-2 transform hover:text-purple-500 hover:scale-110" onClick={()=>handleDelete(row.id)} >
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                    ...
                                                                    <div className="w-full max-w-[570px] rounded-[20px] bg-white py-12 px-8 text-center md:py-[60px] md:px-[70px]">
                                                                        ...
                                                                        <div className=" items-center  justify-center flex  my-4">
                                                                            <div className="w-1/2 px-3 items-center">
                                                                                <button onClick={()=> setDeleteModal(false)} className="text-dark block w-full shadow-2xl rounded-lg border border-[#E9EDF9] p-3 text-center text-base font-medium transition hover:border-red-600 hover:bg-red-600 hover:text-white">Close</button>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                        ...
                        <div x-show="modalOpen" x-transition className="fixed top-0 left-0 flex h-full min-h-screen w-full items-center justify-center bg-[#e5e7eb] bg-opacity-90 px-4 py-5">
                            <div className="w-full max-w-[570px] rounded-[20px] bg-white py-12 px-8 text-center md:py-[60px] md:px-[70px]">
                                ...
                                <div className=" relative ">
                                    ...
                                    <input type="text"  className="my-4 rounded-lg border-transparent flex-1 appearance-none border border-gray-300 w-full py-2 px-4 bg-white text-gray-700 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent" placeholder="DiseaseType Description" onChange={(e)=>setDiseaseTypeDescription(e.target.value)}/>
                                </div>
